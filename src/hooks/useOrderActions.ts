...
export function useOrderActions({
  user, orders, setOrders, ht, firma, firmaId, isAdmin, hesapAktif, showToast,
  ...
  const handleSave = async (form: OrderForm, editingId: string | null) => {
    if (!user) return;
    if (!isAdmin && !hesapAktif) { showToast("Hesabınız aktif değil.", "error"); return; }
  ...
  // 📍 YENİ EKLENDİ: Borç ve Tahsilat Güncelleme
  const handleOdeme = async (id: string, odendi: boolean) => {
    if (!user) return;
    await sbFetch(`siparisler?id=eq.${id}`, { method: "PATCH", prefer: "return=minimal", body: JSON.stringify({ odendi }) }, user.token);
    setOrders((prev) => prev.map((o) => o.id === id ? { ...o, odendi } : o));
    showToast(odendi ? "Tahsilat başarıyla alındı! 💵" : "Borca yazıldı! 📒", "success");
  };

  const handleSms = async ( smsOrder: Siparis | null, durum: string, mesaj: string, kanal: "wa_me" | "wa_api" | "sms", otomatikMi: boolean = false ) => {
    if (!user || !smsOrder) return;
    
    if (kanal === "wa_me" && isStarter) {
      const guncelWaKredi = firma?.wa_kredisi || 0;
      if (guncelWaKredi <= 0) { showToast("Sistem Reddi: WhatsApp bildirim limitiniz dolmuştur.", "error"); return; }
      if (guncelWaKredi > 0) {
        await sbFetch(`firmalar?id=eq.${firmaId}`, { method: "PATCH", prefer: "return=minimal", body: JSON.stringify({ wa_kredisi: guncelWaKredi - 1 }) }, user.token);
      }
    }

    if (kanal === "sms") {
      if (isStarter) return; 
      const netgsmUser = firma?.netgsm_user; const netgsmPass = firma?.netgsm_pass;
      const smsBaslik = firma?.netgsm_baslik || firma?.ad?.slice(0, 11) || "Hali Yikama";

      if (!netgsmUser || !netgsmPass) { if (!otomatikMi) showToast("NetGSM ayarlarınız eksik!", "error"); return; }
      
      let tel = smsOrder.telefon.replace(/[^0-9]/g, "");
      if (tel.startsWith("0")) tel = "9" + tel; else if (tel.startsWith("5")) tel = "90" + tel;

      const params = new URLSearchParams({ usercode: netgsmUser, password: netgsmPass, gsmno: tel, message: mesaj, msgheader: smsBaslik, dil: "TR" });
  ...
  // handleOdeme export edildi
  return { handleSave, handleStatus, handleSms, handleOdeme, handleSil };
}