...
export function OrderModal({ order, ht, firmalar, firma, isAdmin, token, firmaId: propFirmaId, onClose, onSave }: OrderModalProps) {
  ...
  useEffect(() => {
    if (seciliFirmaId && token) {
      sbFetch(`firmalar?id=eq.${seciliFirmaId}&select=hizmet_ili,hizmet_ilceleri`, {}, token)
        .then((res: any) => {
          if (res && res.length > 0) {
      ...
      <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "20px 20px 40px", width: "100%", maxWidth: 600, maxHeight: "92vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "grid", gap: 10, marginBottom: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div ref={musteriRef} style={{ position: "relative" }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: "#64748B", display: "block", marginBottom: 6, textTransform: "uppercase" }}>Müşteri Adı</label>
              <input style={inp} value={musteri} onChange={(e) => musteriDegisti(e.target.value)} onFocus={() => oneriler.length > 0 && setShowOneriler(true)} placeholder="Ad Soyad ara..." autoComplete="off" />
              {showOneriler && (
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 200, background: "#fff", border: "1.5px solid #E2E8F0", borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", overflow: "hidden", marginTop: 4 }}>
                  {oneriler.map((m) => (
                    <div key={m.id} onMouseDown={() => musteriSec(m)} style={{ padding: "12px 16px", cursor: "pointer", borderBottom: "1px solid #F1F5F9" }}>
                      <div style={{ fontWeight: 700, fontSize: 14, color: "#0F172A" }}>👤 {m.ad_soyad}</div>
                      <div style={{ fontSize: 12, color: "#64748B", marginTop: 2 }}>📞 {m.telefon}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: "#64748B", display: "block", marginBottom: 6, textTransform: "uppercase" }}>Telefon</label>
        ...
        <div style={{ background: "#F8FAFC", padding: 16, borderRadius: 12, border: "1px solid #E2E8F0", marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: "#64748B", display: "block", marginBottom: 4 }}>SOKAK & BİNA NO</label>
              <input style={inp} value={acikAdres} onChange={(e) => setAcikAdres(e.target.value)} placeholder="Örn: Hürriyet Sk. No:5 D:2" />
            </div>
          </div>
          
          {adres && !il && !ilce && (
             <div style={{ marginTop: 12 }}>
               <label style={{ fontSize: 11, fontWeight: 700, color: "#EF4444", display: "block", marginBottom: 4 }}>ESKİ ADRES KAYDI</label>
               <input style={{...inp, color: "#64748B", background: "#F1F5F9"}} disabled value={adres} />
             </div>
          )}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          <div>
            <label style={{ fontSize: 11, fontWeight: 700, color: "#64748B", display: "block", marginBottom: 6, textTransform: "uppercase" }}>Tarih</label>
            <input style={inp} type="date" value={tarih} onChange={(e) => setTarih(e.target.value)} max={bugun()} />
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: 700, color: "#64748B", display: "block", marginBottom: 6, textTransform: "uppercase" }}>Notlar</label>
            <input style={inp} value={notlar} onChange={(e) => setNotlar(e.target.value)} placeholder="Zile basmayın vb." />
          </div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {kalemler.length === 0 ? (
            <div style={{ textAlign: "center", padding: "24px", background: "#F8FAFC", borderRadius: 12, border: "1.5px dashed #CBD5E1", color: "#94A3B8", fontSize: 14 }}>Halı kalemi ekleyin</div>
          ) : (
